...
class AVLNode {
    ...
    AVLNode(const Key & theElement, const Value & theVal, AVLNode *lt, AVLNode *rt, int h = 0)
...
private:
    ...
    //helper functions that are need for AVL implementation
...
//COPY CONSTRUCTOR
template<class Key, class Value>
AVLSearchTree<Key, Value>::AVLSearchTree(const AVLNode<Key, Value> &rhs) : root(nullptr),
...
template<class Key, class Value>
bool AVLSearchTree<Key, Value>::contains(const Key &x) const {
...
AVLNode<Key,Value>* AVLSearchTree<Key, Value>::insert(const Key& x, const Value& v, AVLNode<Key, Value>* & t) {
    ...
    } else { // Word already exists in the tree
        // Check if the document already exists for this word
        auto it = find_if(t->value.documentItems.begin(), t->value.documentItems.end(),
                          [&v](const DocumentItem& item) { return item.documentName == v.documentItems[0].documentName; });

        if (it != t->value.documentItems.end()) {
            // Document exists, increment count
            it->count++;
        } else {
            // Document doesn't exist, add new DocumentItem
            t->value.documentItems.push_back(v.documentItems[0]);
        }
        return t; // No need for rotation as the word already exists
    }
